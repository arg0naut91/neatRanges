#' Collapses the consecutive date/time ranges into one record.
#'
...
#' @param max_gap Gap between date/time ranges, e.g. for 0, default, it will put together all records where there is no gap in-between
#' @param dimension Indicate whether your range includes only dates ('date') or also time ('time'). Defaults to 'date'
#' @param fmt The format of your date/time field, defaults to '%Y-%m-%d'.
#'
...
#' @export
collapse_ranges <- function(df, groups, start_var, end_var, range = "date", max_gap = 0L, fmt = "%Y-%m-%d") {
  ...
  if (range == "date") {
    ...
    if (fmt == "%Y-%m-%d") { 
      ...
      warning("Dimension 'time' selected but format unchanged. Will try to convert to '%Y-%m-%d %H:%M:%OS'.")
